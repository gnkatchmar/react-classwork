...
describe('array', () => {
    ...
    it('sort', () => {
        ...
        words.sort();
        assert.deepEqual(words, ['bar', 'foo', 'qux']);

        const numbers = [2, 4, 10, 5];
        numbers.sort((a, b) => a-b);
        assert.deepEqual(numbers, [2, 4, 5, 10])
    })
    ...
    describe('predicate', () => {
        ...
        it('filter', () => {
            ...
            assert.deepEqual(numbers, [1, 5, 30, 7, 23]);
            assert.deepEqual(filtered, [1, 5, 7]);
        });
        ...
        it('findIndex', () => {
            ...
            assert.equal(indexOf, 3);
            const notIndexOf = numbers.indexOf(-53);
            assert.equal(notIndexOf, -1);
        })
    ...
    describe('foreach/map', () => {
        ...
        it('forEach', () => {
            ...
            assert.deepEqual(people, [{ name: 'name0' }, { name: 'name1' }, { name: 'name2' }])
        });
        ...
        it('map', () => {
            ...
            assert.deepEqual(newNumbers, [2, 10, 60, 14, 46]);
            ...
            assert.deepEqual(objs, [{ a: 1 }, { b: 5 }, { c: 30 }, { d: 7 }, { e: 23 }]);
        });
        ...
        it('map has holes', () => {
            ...
            assert.equal(mapped.length, holey.length);
            assert.equal(count, 4)
        });
    });
    ...
    describe('chaining', () => {
        ...
        it('chaining filter, map', () => {
            const objsOver10 = numbers
                .filter(n => n > 10) // [30, 23]
                .map(n => ({ number: n })); // [{ n: 30 }, { n: 23 }]

            assert.deepEqual(objsOver10, [{ number: 30 }, { number: 23 }])
        });
    });
